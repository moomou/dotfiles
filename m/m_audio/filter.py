...
class AudioFilter(Base):
    ...
    def _remove_silence_vad(
        self, sr, signal, frame_duration=None, frame_shift=None, threshold=0.25
    ):
        """
    ...
    def remove_silence_vad(
        ...
        filtered_sig = self._remove_silence_vad(
            sr, signal, frame_duration, frame_shift, threshold
        )
    ...
    def remove_vocal_exp2(self, in_file, in_file2):
        ...
        mask_i = librosa.util.softmask(S_full, 2 * (S_full - S_filter), power=2)
    ...
    def remove_vocal_exp(
        self, in_file, out_file=None, frame_duration=None, frame_shift=None
    ):
        audio_util = self._module("m_audio.audio_util")
        librosa = self._module("librosa")
        np = self._module("numpy")
        ...
        S_filter, phase = librosa.magphase(librosa.stft(librosa.to_mono(s_filter)))
    ...
    def repeat_sim(self, in_file):
        ...
        if self._debug_mode():
            ...
            librosa.display.specshow(
                librosa.amplitude_to_db(S_full[:, idx], ref=np.max), y_axis="log", sr=sr
            )
