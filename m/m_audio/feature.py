...
class AudioFeature(Base):
    def __init__(self):
        super().__init__(["librosa", "h5py", "numpy", "m_audio.audio_util"])
    ...
    def extract_mfcc(
        self, in_file, h5_id=None, out_h5=None, prefix="mfcc_13", sr=None, max_freq=8000
    ):
        ...
        if out_h5:
            with h5py.File(out_h5, mode="a") as h5:
                ...
                mfcc_grp.create_dataset(row_id, data=mfcc_feature, dtype="float32")
        else:
            self._logger.info("sig:: %s, %s" % (sig, sig.shape))
            self._logger.info("mfcc:: %s, %s" % (mfcc_feature, mfcc_feature.shape))
            self._logger.info("id:: %s", row_id)
